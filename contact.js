...
//     // --- 1. ГЕНЕРАЦИЯ ВРЕМЕНИ (с 09:00 до 20:00, интервал 15 мин) ---
//     if (timeSelect) {
//         for (let hour = 9; hour <= 20; hour++) {
//             [0, 15, 30, 45].forEach(min => {
//                 if (hour === 20 && min > 0) return; // Ограничение до 20:00
//                 let h = hour < 10 ? '0' + hour : hour;
//                 let m = min === 0 ? '00' : min;
//                 let option = document.createElement('option');
//                 option.value = `${h}:${m}`;
...
//     // --- 2. ОГРАНИЧЕНИЕ ДАТЫ (минимум сегодня) ---
//     if (dateInput) {
//         dateInput.setAttribute('min', new Date().toISOString().split('T')[0]);
//     }
...
//         let res = '';
//         if (val.length > 0) res = '+7 (' + val.substring(0, 3);
//         if (val.length > 3) res += ') ' + val.substring(3, 6);
//         if (val.length > 6) res += '-' + val.substring(6, 8);
//         if (val.length > 8) res += '-' + val.substring(8, 10);
...
//     // --- 5. МАГНИТНЫЙ ЭФФЕКТ КНОПКИ (для ПК) ---
...
//             // Проверка на блокировку администратором (/black)
//             if (name.startsWith('/black')) {
//                 let blackList = JSON.parse(localStorage.getItem('adminBlackList')) || [];
//                 blackList.push(dateTimeKey);
//                 localStorage.setItem('adminBlackList', JSON.stringify(blackList));
...
//                         status.innerText = "✅ Запись создана! ✨";
//                         status.style.color = "gold";
...
//                         }, 3000);
//                     } else {
//                         throw new Error("Ошибка при отправке одному из ботов");
//                     }
//                 })
//                 .catch(err => {
//                     status.innerText = "❌ Ошибка при отправке. Попробуйте снова.";
//                     status.style.color = "red";
//                 })
//                 .finally(() => {
//                     btn.disabled = false;
